...
int CCrashHandler::Init(
		...
		int nRestartTimeout,
		LPCTSTR lpcszCompressionPassword)
{ 
	...
	if (lpcszCompressionPassword != NULL)
		m_sCompressionPassword = lpcszCompressionPassword;
...
CRASH_DESCRIPTION* CCrashHandler::PackCrashInfoIntoSharedMem(CSharedMem* pSharedMem, BOOL bTempMem)
{
	...
	m_pTmpCrashDesc->m_dwCompressionPasswordOffs = PackString(m_sCompressionPassword);
	// Pack file items
