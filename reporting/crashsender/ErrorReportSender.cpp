...
{ 
    ...
    { 
		...
		if (m_CrashInfo.m_sCompressionPassword.GetLength() > 0)
		{
			void* buf = (void*)malloc(8192);

			unsigned long CRC;

			int err = getFileCrc((const char*)strconv.t2a(sFileName.GetBuffer(0)), buf, 8192, &CRC);
		
			n = zipOpenNewFileInZip3(hZip, (const char*)strconv.t2a(sDstFileName.GetBuffer(0)), &info,
				NULL, 0, NULL, 0, strconv.t2a(sDesc), Z_DEFLATED, Z_DEFAULT_COMPRESSION, 0,
				-MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, (const char*)strconv.t2a(m_CrashInfo.m_sCompressionPassword.GetBuffer(0)), CRC);
		}
		else
		{
			n = zipOpenNewFileInZip(hZip, (const char*)strconv.t2a(sDstFileName.GetBuffer(0)), &info,
